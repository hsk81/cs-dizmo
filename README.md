...
Create the `CsDizmo` dizmo by running `npm run build` and drag and drop the generated file &ndash; which can be found under `build/CsDizmo-x.y.z.dzm` &ndash; onto the *dizmoViewer* application.
...
Packages the dizmo as `build/CsDizmo-x.y.z.dzm` by running all required build tasks and compressing the resulting dizmo as a ZIP archive (but with a `.dzm` extension). Runs also if required `npm install`:
